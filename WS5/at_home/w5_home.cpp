...
int main(int argc, char** argv)
{
	...
	for (count = 0; count < 4; count++) {
		...
		if (file) {
			if (book[0] != '#') { //book[0] is first line in the books
	...
	auto price = [usdToCadRate, gbpToCadRate](Book& book) { // or [&]
		if (book.country() == "US") {
	...
	try {
		...
		// NOTE: has to use out_of_rage because operator[] in Collection.h
		//NOTE: std::out_of range inherits the what member function from logic_error.
	}
	...
	try {
		...
		// NOTE: not relying on out_of_range in functor
	}
}
